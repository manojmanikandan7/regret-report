...
analysis. \textbf{EvoX}~\cite{google_vizier} is a JAX-based
...
\begin{table}[ht]
  \centering
  \caption{\acrshort{cli} sub-commands exposed by the
...
The software was intended to be extensive \& configurable for other
...
abstract classes \texttt{Problem} \& \texttt{Algorithm}, ensuring a
...
redundant stylistic whitespaces and newlines with other seperators.
This reduced the raw file sizes by upto 50\%.
...
\begin{figure}[p]
  ...
  \caption{Simplified UML class diagram of the \texttt{regret} framework.
    ...
    Several concrete subclasses (e.g.\ \texttt{MuPlusLambdaEA},
      \texttt{\acrshort{rls}Exploration}, \texttt{NKLandscape},
...
\begin{table}[H]
  ...
  \begin{tabular}{p{4cm}p{8cm}p{2cm}}
    \hline
    \textbf{Plot} & \textbf{Output filename pattern - \texttt{pdf}
