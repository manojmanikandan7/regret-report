...
absence of readily available software infrastructure that combines (i)~flexible, declarative
...
environment for studying \acrfull{sr}, \acrfull{ir}, and \acrfull{cr} across
...
Runtime profile CSV artifacts are written to a \texttt{profiles/} subdirectory
within the raw results tree, while runtime profile plots are written to the
corresponding \texttt{profiles/} subdirectory under \texttt{results/figures/}.
All figures are saved as \acrshort{pdf} at 300\,\acrshort{dpi}.
...
\begin{longtable}{p{0.55\linewidth}p{0.45\linewidth}}
  ...
  \texttt{.../n<N>/profiles/}
  & Runtime profile plots: inverse runtime profile surface/curves,
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.55\linewidth]{red-green-refactor.png}
  \caption[Red--Green--Refactor \acrshort{tdd}
  technique]{Red--Green--Refactor \acrshort{tdd} technique. Start by
    writing a test that might fail (the red step). Next, write code that is
...
using a red--green--refactor workflow
(Figure~\ref{fig:red-green-refactor}): tests were written prior to or alongside
...
\begin{longtable}{p{3cm}p{3cm}p{8cm}}
  ...
  instances (e.g.\ Petersen coloring) \\
  \hline
