...
experiment dispatch, (ii)~trajectory-level data collection, and
...
runtime profile analysis, or the configuration-first dispatch model required
...
\acrshort{yaml} configuration files and dispatches the appropriate sub-command.
...
When \texttt{parallel: true} is set, independent runs are dispatched via
...
The software was designed to be extensive \& configurable for other developers and users to interact with quickly, with options for modularity when required. 
...
\subsubsection{Dispatching framework}
...
orchestration layer (\texttt{regret.experiments.orchestration}) coordinates
problem instantiation, algorithm dispatch, result persistence, and plot
...
\begin{figure}[ht]
\caption[Configuration parse pipeline]{Validation and dispatch pipeline. \acrshort{yaml} configuration files are
...
TTFO distribution        & \texttt{history\_ttfo\_markers}               & No \\
...
The test suite follows a \textbf{Test-Driven Development} (TDD) discipline
using a red--green workflow: tests were written prior to or alongside
implementation and serve as executable specifications. Coverage is measured
using \texttt{coverage}~\cite{coverage} with branch tracking enabled, and a
