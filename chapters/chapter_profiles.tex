...
$\{\tau_v > t\}$:
the level $v$ has not yet been hit. The incumbent cumulative regret
over a budget of
...
The following identity connects \textit{expected} cumulative regret
...
Theorem~\ref{thm:regret-profile} expresses expected cumulative regret
as a double sum
...
expected cumulative regret decomposes as the sum of the expected
first hitting times
...
Pinto and Doerr~\cite{pinto2018practice} note that runtime profiles
are particularly
...
bound on expected cumulative regret, and conversely, lower bounds on
regret propagate to
lower bounds on a weighted aggregate of first hitting times.
Cumulative regret thus
